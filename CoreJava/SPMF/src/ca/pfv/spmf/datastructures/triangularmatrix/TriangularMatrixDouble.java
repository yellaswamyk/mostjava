...
/**
 * This class is for creating a triangular matrix of doubles represented by using arrays.
 * All the elements in the matrix are initialized to zero and can
 * be changed to other double values.
 * For example: <br/><br/>
 * 
 * 0: [0, 0, 0, 0]<br/>
 * 1: [0, 0, 0]<br/>
 ...
 * <br/><br/>
 * 
 * This structure is used by various data mining algorithms.
 * 
 * @author Philippe Fournier-Viger
 */
public class TriangularMatrixDouble implements AbstractTriangularMatrixDouble {
	
	// the triangular matrix is a two dimension array of doubles
	 ...
	 */
	public TriangularMatrixDouble(int elementCount){
		// save the number of lines
		this.elementCount = elementCount;
		// initialize the matrix
		matrix = new double[elementCount-1][]; // -1 cause we want it shorter of 1 element
		for(int i=0; i< elementCount-1; i++){ // -1 cause we want it shorter of 1 element
		   // allocate an array for each row
			matrix[i] = new double[elementCount - i -1];
		}
	}
	...
	public static void main(String[] args) {
		TriangularMatrixDouble a = new TriangularMatrixDouble(5);
	 ...
	 */
	public void incrementCount(int i, int j, double value) {
	 ...
	 */
	public double getSupportForItems(int i, int j){
		if(j < i){
	 ...
	 */
	public void setSupport(Integer i, Integer j, double support) {
		if(j < i){
