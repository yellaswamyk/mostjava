...
/**
 * Fix a transaction database with utility information by adding an increment (e.g. 1) to
 * all the item names. If the increment is 1, this means that if an item is called 1, it will be 
 * called 2 in the output. And if an item is called 5, it will be called 6
 * and so on.
...
public class FixItemIDsTransactionDatabaseWithUtilityTool {
	...
	/**
	 * Fix a transaction database to increase all the item names by an increment (e.g. 1).
