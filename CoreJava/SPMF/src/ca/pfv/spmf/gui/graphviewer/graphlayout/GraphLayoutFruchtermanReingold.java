 ...
 * algorithm known as the Fruchterman/Reingold (1991) algorithm, which is a force directed graph layout algorithm.
 * This is used by the GraphViewerPanel of SPMF.
 * 
...
public class GraphLayoutFruchtermanReingold extends AbstractGraphLayout {
	...
	double STARTING_TEMPERATURE = 20;
	...
	public void autoLayout(List<GEdge> edges, List<GNode> nodes, int canvasWidth, int canvasHeight) {
		...
		// It is always initialized using the same number on purpose so that the generated graph is consistent between each execution.
...
//			System.out.println("iteration " + i +" time " + elapsed + " ms");
	...
	protected class GraphLayoutFruchtermanReingoldState {
		...
		public GraphLayoutFruchtermanReingoldState(List<GNode> nodes, int canvasWidth, int canvasHeight) {
			...
			// outside the canvas.
			maxX = canvasWidth - GNode.RADIUS;
			maxY = canvasHeight - GNode.RADIUS;

			// Calculate the real size of the canvas after we remove a margin
			// that has the size of the radius.
			realWidth = maxX - GNode.RADIUS;
			realHeight = maxY - GNode.RADIUS;
		...
		protected void initializePositions(Random random) {
			...
			for (int vIndex = 0; vIndex < nodeCount; vIndex++) {
				newPosX[vIndex] = GNode.RADIUS + (realWidth * random.nextDouble());
				newPosY[vIndex] = GNode.RADIUS + (realHeight * random.nextDouble());
			}
		}
		...
		protected void updatePositions(double[] displacementX, double[] displacementY) {
			...
			for (int vIndex = 0; vIndex < nodeCount; vIndex++) {
				...
				// canvas
				if (newPosX[vIndex] < GNode.RADIUS) {
					newPosX[vIndex] = GNode.RADIUS;
				...
				// canvas
				if (newPosY[vIndex] < GNode.RADIUS) {
					newPosY[vIndex] = GNode.RADIUS;
